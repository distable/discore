...
# region Emits
def print_invoke(funcname):
    log(f'--> {session.name}.{script_name}.{funcname}')


def _invoke_safe(*args, **kwargs):
    if 'unsafe' not in kwargs:
        kwargs['unsafe'] = enable_unsafe
    return invoke_safe(*args, **kwargs)


@trace_decorator
def _script_emit(name, unsafe=False, **kwargs):
    set_lib_refs()
    print_invoke(name)

    if hasattr(script, name):
        func = getattr(script, name)
        _invoke_safe(func, unsafe=unsafe, **kwargs)
    if hasattr(script, 'callback'):
        _invoke_safe(script.callback, name, unsafe=unsafe, **kwargs)
...
def invoke_init():
    ...
    if init_state.value < InitState.HEAVY.value and not enable_dev:
        try:
            _script_emit('init_heavy', unsafe=True)
...
def invoke_frame():
    ...
    if rv.promptnode is not None and not enable_dev:
        from src.party.ravelang import pnodes
        rv.prompt = pnodes.eval_prompt(rv.promptnode, rv.t)
        # TODO we need a switch for bake/update mode
    ...
    with trace('invoke_frame.invocation'):
        last_frame_error = _script_emit('frame', failsleep=0.25)
