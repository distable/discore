...
def load_pose_frames(path, joint: Union[int, tuple], original_fps=29.89):
    ...
    framecount = len(o)
    stretchcount = int(framecount / original_fps * rv.fps)
...
def load_vr(path, beta=0.5, min_cutoff=0.004, one_euro=True):
    ...
    with open(path, 'r') as file:
        ...
        for i in range(len(inputs)):
            ...
            d1 = resampy.resample(d1, rec.samples_per_second, rv.fps)
...
def load_midibucket_frames(path, as_numpy=False):
    ...
    framecount = int(duration * rv.fps)
    frames = dict()
    lasttimes = dict()
    for n in notes:
        lasttimes[n] = 0
...
def load_midi_frames(path):
    ...
    # Build the frame data
    frames = np.zeros(int(duration * rv.fps))
    time = 0
    lastf = 0
    lastn = 0
    for msg in mid:
...
def make_temporal_markers(like, measure_duration, start=0.0, end=np.inf):
    ...
    for ts in np.arange(start * rv.fps, np.min((end, s.shape[0])), measure_duration * rv.fps):
        s[int(ts)] = 1
    return s
...
def make_msec(like, sections, filter=None, step=None, coded=True, sustain=False):
    ...
    for isec in range(len(sections) - 1):  # we expect an end marker at the very end
        ...
        if filter is None or label == filter:
            ...
            if step is None:
                if not sustain:
                    s[int(time * rv.fps)] = code
                else:
                    s[int(time * rv.fps):int(time1 * rv.fps)] = code
            else:
                i = 0
                t = time
                while t < time1:
