...
from src.party.models.ted import TED
...
class AnyLine:
    ...
    def load_model(self):
        ...
        model = TEDDetector(model).to(self.device)
        return model

    def get_anyline(self, image):
        image = convert.load_torch(image)

        # Process the image with MTEED model
        if self.model is None:
            self.model = self.load_model()

        mteed_result = tricks.common_annotator_call(self.model, image, resolution=image.shape[1])
        ...
        # del mteed_model
        # ret = (torch.tensor(mteed_result).unsqueeze(0),)

        # convert.load_cv2(ret[0])
        # return ret

        ret:np.ndarray = (mteed_result[2] * 255)
